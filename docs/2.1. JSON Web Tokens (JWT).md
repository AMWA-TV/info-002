...
The Payload contains the token claims, including when the token became active, when it is due to expire, who the token is intended for, and some other standard registered claims. 

#### Standard Claims

The [IS-10 specification](https://specs.amwa.tv/is-10/branches/main/docs/4.4._Behaviour_-_Access_Tokens.html#registered-claims) gives details of how NMOS API Clients and Servers should use registered claims. In addition to those listed in the specification, others claims of interest are given below:
