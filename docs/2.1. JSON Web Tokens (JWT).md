...
Payload contains the token claims, including when the token became active, when it is due to expire, who the token is intended for, and some other registered (standard) and private claims.

#### Registered Claims

The [IS-10 specification](https://specs.amwa.tv/is-10/branches/main/docs/4.4._Behaviour_-_Access_Tokens.html#registered-claims) gives details of how NMOS API Clients and Servers should use registered claims. In addition to those listed in the specification, other claims of interest are given below:
...
| jti | The `jti` (JWT ID) claim MAY be included in the token. It is a case-sensitive unique identifier of the token even across different issuers. |

#### Private Claims

IS-10 has defined some private claims, related to the NMOS scopes. These are configured in the Authorization Server and are covered in more detail in [that section](3.2.%20NMOS%20Private%20Claims.md).
